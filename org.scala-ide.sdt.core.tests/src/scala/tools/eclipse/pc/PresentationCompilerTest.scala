...
class PresentationCompilerTest {
  ...
  def unattributedParseTree_t1001326() {
    val cu = scalaCompilationUnit("t1001326/ParsedTree.scala")
    val noSymbols = project.withSourceFile(cu) { (sf, compiler) =>
      noSymbolsOrTypes(compiler)(compiler.parseTree(sf))
    } (false)
  ...
  def neverModifyParseTree_t1001326() {
    ...
    val notChanged = project.withSourceFile(cu) { (sf, compiler) =>
      val parseTree = compiler.parseTree(sf)
      open(path)
      noSymbolsOrTypes(compiler)(parseTree)
    } (false)
  ...
  /**
   * Traverses a tree and makes sure that there are no types or symbols present in the tree. See
   * in-line comments for one exception.
   */
  private def noSymbolsOrTypes(compiler: ScalaPresentationCompiler)(t: compiler.Tree): Boolean = {
    import compiler._
    t.forAll {
      // Currently some of the AST nodes that are generated by the
      // compiler will contain symbols. for now we ignore those nodes.
      case Ident(nme.scala_) => true
      // No other nodes are allowed to contain symbols though.
      case t => {
        (t.symbol == null || t.symbol == NoSymbol) &&
        (t.tpe == null || t.tpe == NoType)
      }
    }
  }
