 ...
 */
trait BitwiseShiftRightWithZeros[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def >>>[OtherProxy <: IntegerNumberJdiProxy[_, OtherProxy], Res <: IntegerNumberJdiProxy[_, Res]](other: OtherProxy)(implicit bitOpRes: BitShiftRes[Proxy, OtherProxy, Res]): Res =
    BitwiseShiftRightWithZeros.operationHelper(proxyContext).applyOperation(self, other).asInstanceOf[Res]
}

object BitwiseShiftRightWithZeros {
  protected def operationHelper(ctx: JdiContext) = new BitwiseShiftHelper(ctx: JdiContext) {
