 ...
 * Value is bit-shifted left by the specified number of bits, filling in the new right bits with zeros.
 */
trait BitwiseShiftLeftWithZeros[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>

  def <<[OtherProxy <: IntegerNumberJdiProxy[_, OtherProxy], Res <: IntegerNumberJdiProxy[_, Res]](other: OtherProxy)(implicit bitOpRes: BitShiftRes[Proxy, OtherProxy, Res]): Res =
    BitwiseShiftLeftWithZeros.operationHelper(proxyContext).applyOperation(self, other).asInstanceOf[Res]
}

object BitwiseShiftLeftWithZeros {
  protected def operationHelper(ctx: JdiContext) = new BitwiseShiftHelper(ctx) {
