 ...
 */
trait BitwiseShiftLeftWithZeros {
  self: IntegerNumberJdiProxy[_, _] =>

  def <<(other: IntegerNumberJdiProxy[_, _]): IntegerNumberJdiProxy[_, _] =
    BitwiseShiftLeftWithZeros.operationHelper(context).applyOperation(self, other)
}

object BitwiseShiftLeftWithZeros {
  protected def operationHelper(ctx: JdiContext) = new BitwiseOperationHelper(ctx) {
