 ...
 */
trait BitwiseShiftRight[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def >>[OtherProxy <: IntegerNumberJdiProxy[_, OtherProxy], Res <: IntegerNumberJdiProxy[_, Res]](other: OtherProxy)(implicit bitOpRes: BitShiftRes[Proxy, OtherProxy, Res]): Res =
    BitwiseShiftRight.operationHelper(proxyContext).applyOperation(self, other).asInstanceOf[Res]
}

object BitwiseShiftRight {
