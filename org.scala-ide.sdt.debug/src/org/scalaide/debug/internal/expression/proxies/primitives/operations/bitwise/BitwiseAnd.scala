...
/**
 * Implements bitwise AND for primitive proxies.
 */
trait BitwiseAnd[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def &[OtherProxy <: IntegerNumberJdiProxy[_, OtherProxy], Res <: IntegerNumberJdiProxy[_, Res]](other: OtherProxy)(implicit bitOpRes: BitOpRes[Proxy, OtherProxy, Res]): Res =
    BitwiseAnd.operationHelper(proxyContext).applyOperation(self, other).asInstanceOf[Res]
}
