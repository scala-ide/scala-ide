...
/**
 * Implements integer subtraction for primitive proxies.
 */
trait IntegerSubtraction {
  self: IntegerNumberJdiProxy[_, _] =>

  def -(other: IntegerNumberJdiProxy[_, _]) = Subtraction.operationHelper(context).applyIntegerOperation(self, other)
}

/**
 * Implements floating point subtraction for primitive proxies.
 */
trait FloatingPointSubtraction {
  self: NumberJdiProxy[_, _] =>

  def -(other: NumberJdiProxy[_, _]) = Subtraction.operationHelper(context).applyFloatingPointOperation(self, other)
}

object Subtraction {
  private[operations] def operationHelper(context: JdiContext) = new NumericOperationHelper(context) {
