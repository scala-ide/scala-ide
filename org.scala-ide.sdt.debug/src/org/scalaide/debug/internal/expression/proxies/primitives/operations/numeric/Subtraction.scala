 ...
 */
trait IntegerSubtraction[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def -[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Subtraction.operationHelper(proxyContext).applyIntegerOperation(self, other).asInstanceOf[Res]
}

/**
 * Implements floating point subtraction for primitive proxies.
 */
trait FloatingPointSubtraction[Proxy <: NumberJdiProxy[_, Proxy]] {
  self: Proxy =>

  def -[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Subtraction.operationHelper(proxyContext).applyFloatingPointOperation(self, other).asInstanceOf[Res]
}
