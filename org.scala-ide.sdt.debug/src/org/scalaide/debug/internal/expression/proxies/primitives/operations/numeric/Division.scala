 ...
 */
trait IntegerDivision[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def /[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Division.operationHelper(proxyContext).applyIntegerOperation(self, other).asInstanceOf[Res]
}

/**
 * Implements floating point division for primitive proxies.
 */
trait FloatingPointDivision[Proxy <: NumberJdiProxy[_, Proxy]] {
  self: Proxy =>

  def /[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Division.operationHelper(proxyContext).applyFloatingPointOperation(self, other).asInstanceOf[Res]
}
