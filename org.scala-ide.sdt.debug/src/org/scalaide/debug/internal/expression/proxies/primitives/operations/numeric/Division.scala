...
/**
 * Implements integer division for primitive proxies.
 */
trait IntegerDivision {
  self: IntegerNumberJdiProxy[_, _] =>

  def /(other: IntegerNumberJdiProxy[_, _]) = Division.operationHelper(context).applyIntegerOperation(self, other)
}

/**
 * Implements floating point division for primitive proxies.
 */
trait FloatingPointDivision {
  self: NumberJdiProxy[_, _] =>

  def /(other: NumberJdiProxy[_, _]) = Division.operationHelper(context).applyFloatingPointOperation(self, other)
}

object Division {
  private[operations] def operationHelper(context: JdiContext) = new NumericOperationHelper(context: JdiContext) {
