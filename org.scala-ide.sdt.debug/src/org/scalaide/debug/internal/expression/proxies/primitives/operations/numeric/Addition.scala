 ...
 */
trait IntegerAddition[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def +[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Addition.operationHelper(proxyContext).applyIntegerOperation(self, other).asInstanceOf[Res]
}

/**
 * Implements floating point addition for primitive proxies.
 */
trait FloatingPointAddition[Proxy <: NumberJdiProxy[_, Proxy]] {
  self: Proxy =>

  def +[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Addition.operationHelper(proxyContext).applyFloatingPointOperation(self, other).asInstanceOf[Res]
}
