 ...
 */
trait IntegerMultiplication {
  self: IntegerNumberJdiProxy[_, _] =>

  def *(other: IntegerNumberJdiProxy[_, _]) = Multiplication.operationHelper(context).applyIntegerOperation(self, other)
}

/**
 * Implements floating point multiplication for primitive proxies.
 */
trait FloatingPointMultiplication {
  self: NumberJdiProxy[_, _] =>

  def *(other: NumberJdiProxy[_, _]) = Multiplication.operationHelper(context).applyFloatingPointOperation(self, other)
}
