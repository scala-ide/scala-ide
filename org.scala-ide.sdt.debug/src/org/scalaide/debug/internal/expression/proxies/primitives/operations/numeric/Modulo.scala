...
/**
 * Implements integer modulo for primitive proxies.
 */
trait IntegerModulo {
  self: IntegerNumberJdiProxy[_, _] =>

  def %(other: IntegerNumberJdiProxy[_, _]) = Modulo.operationHelper(context).applyIntegerOperation(self, other)
}

/**
 * Implements floating point modulo for primitive proxies.
 */
trait FloatingPointModulo {
  self: NumberJdiProxy[_, _] =>

  def %(other: NumberJdiProxy[_, _]) = Modulo.operationHelper(context).applyFloatingPointOperation(self, other)
}

object Modulo {
  private[operations] def operationHelper(ctx: JdiContext) = new NumericOperationHelper(ctx: JdiContext) {
