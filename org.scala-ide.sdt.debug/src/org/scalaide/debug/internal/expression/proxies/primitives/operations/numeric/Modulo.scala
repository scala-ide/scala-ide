 ...
 */
trait IntegerModulo[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def %[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Modulo.operationHelper(proxyContext).applyIntegerOperation(self, other).asInstanceOf[Res]
}

/**
 * Implements floating point modulo for primitive proxies.
 */
trait FloatingPointModulo[Proxy <: NumberJdiProxy[_, Proxy]] {
  self: Proxy =>

  def %[OtherProxy <: NumberJdiProxy[_, OtherProxy], Res <: NumberJdiProxy[_, Res]](other: OtherProxy)(implicit res: NumOpRes[Proxy, OtherProxy, Res]): Res =
    Modulo.operationHelper(proxyContext).applyFloatingPointOperation(self, other).asInstanceOf[Res]
}
