...
/**
 * Implements bitwise OR for primitive proxies.
 */
trait BitwiseOr[Proxy <: IntegerNumberJdiProxy[_, Proxy]] {
  self: Proxy =>
  ...
  def |[OtherProxy <: IntegerNumberJdiProxy[_, OtherProxy], Res <: IntegerNumberJdiProxy[_, Res]](other: OtherProxy)(implicit bitOpRes: BitOpRes[Proxy, OtherProxy, Res]): Res =
    BitwiseOr.operationHelper(proxyContext).applyOperation(self, other).asInstanceOf[Res]
}
