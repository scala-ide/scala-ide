 ...
 */
class ImportElement(val dependedOn: Option[ThisElement], val creationCode: Option[TermName] => Option[String])
...
final case class ThisTransformation private(thisHistory: Seq[ThisElement], importHistory: Seq[ImportElement]) {
  ...
  def fromField(fieldName: String, from: ReferenceType): ThisTransformation = {
    ...
    def code(valName: Option[TermName]) =
  ...
  private[context] lazy val nameMap: Map[ThisElement, TermName] = thisHistory.zipWithIndex.map {
    case (thisElement, 0) => thisElement -> TermName(Names.Debugger.thisValName)
    case (thisElement, nr) => thisElement -> TermName(Names.Debugger.thisValName + "_" + nr)
  }(collection.breakOut)

  /** Maps names to elements */
  private[context] lazy val elementMap: Map[TermName, ThisElement] = nameMap.map(_.swap)
}
