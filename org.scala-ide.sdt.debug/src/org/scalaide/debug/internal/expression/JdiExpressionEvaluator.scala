...
class JdiExpressionEvaluator(
  val debugTarget: ScalaDebugTarget,
  monitor: ProgressMonitor = NullProgressMonitor,
  debugState: DebugState = GlobalDebugState)
  extends ExpressionEvaluator(JdiExpressionEvaluator.classloaderForRun(debugTarget), monitor) {
...
protected object JdiExpressionEvaluator extends HasLogger {
  ...
  def classloaderForRun(debugTarget: ScalaDebugTarget): ClassLoader = {
    val classpathEntries = debugTarget.classPath.getOrElse(Nil)
