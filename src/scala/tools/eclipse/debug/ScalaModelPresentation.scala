...
class ScalaModelPresentation extends JDIModelPresentation {
  ...
  override def getEditorInput(that : AnyRef) = {
    if (ScalaPlugin.plugin != null) 
      ScalaPlugin.plugin.inputFor(that) getOrElse (that match {
      case that : JavaLineBreakpoint => 
        ...
        else ScalaPlugin.plugin.inputFor(JavaCore.create(tpe)) getOrElse super.getEditorInput(that)
