...
class DivisionTest extends BaseIntegrationTest(DivisionTest) {
  ...
  def `division by 0.0`(): Unit = {
    eval("int / 0.0", int / 0.0, JavaBoxed.Double)
    eval("1 / 0.0", 1 / 0.0, JavaBoxed.Double)
    eval("-int / 0.0", -int / 0.0, JavaBoxed.Double)
    eval("-1 / 0.0", -1 / 0.0, JavaBoxed.Double)
    eval("0 / 0.0", 0 / 0.0, JavaBoxed.Double)
  }

  def `number / 0.0f`(): Unit = {
    eval("1 / 0.0f", 1 / 0.0f, JavaBoxed.Float)
    eval("-1 / 0.0f", -1 / 0.0f, JavaBoxed.Float)
    eval("0 / 0.0f", 0 / 0.0f, JavaBoxed.Float)
  }

  @Ignore("TODO - O-4549 - add support for float literals")
  @Test
  def `variable / 0.0f - `(): Unit = {
    eval("int / 0.0f", int / 0.0f, JavaBoxed.Float)
    eval("-int / 0.0f", -int / 0.0f, JavaBoxed.Float)
  }
}
