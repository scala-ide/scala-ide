...
class ObjectsTest extends BaseIntegrationTest(ObjectsTest) {
  ...
  @Test
  def testNestedObject(): Unit = disableOnJava8 { eval("LibObject.LibNestedObject.id", "2", Java.boxed.Integer) }

  @Test
  def testMoreNestedObject(): Unit = eval("LibObject.LibNestedObject.LibMoreNestedObject.id", "3", Java.boxed.Integer)

  @Test
  def testNestedObjectWithVal(): Unit = disableOnJava8 { eval("LibObject.nestedClass.LibMoreNestedObject.id", "4", Java.boxed.Integer) }
