...
class DivisionTest extends BaseIntegrationTest(DivisionTest) with DefaultBeforeAfterEach {
  ...
  def `division by 0.0`(): Unit = {
    eval("int / 0.0", int / 0.0, Java.primitives.double)
    eval("1 / 0.0", 1 / 0.0, Java.primitives.double)
    eval("-int / 0.0", -int / 0.0, Java.primitives.double)
    eval("-1 / 0.0", -1 / 0.0, Java.primitives.double)
    eval("0 / 0.0", 0 / 0.0, Java.primitives.double)
  }

  def `number / 0.0f`(): Unit = {
    eval("1 / 0.0f", 1 / 0.0f, Java.primitives.float)
    eval("-1 / 0.0f", -1 / 0.0f, Java.primitives.float)
    eval("0 / 0.0f", 0 / 0.0f, Java.primitives.float)
  }

  @Test
  def `variable / 0.0f - `(): Unit = {
    eval("int / 0.0f", int / 0.0f, Java.primitives.float)
    eval("-int / 0.0f", -int / 0.0f, Java.primitives.float)
  }
}
