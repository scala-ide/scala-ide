...
trait InterpreterConsoleView { self: ViewPart =>
  protected case class ColorRGB(r: Int, g: Int, b: Int) {
    def apply(display: Display) = new Color(display, r, g, b)
  }

  protected var textWidget: StyledText = null
  protected var codeBgColor = ColorRGB(230, 230, 230) // light gray
  protected var codeFgColor = ColorRGB(64, 0, 128) // eggplant
  protected var errorFgColor = ColorRGB(128, 0, 64) // maroon
   ...
   */
  protected def displayCode(text: String) = displayPadded(codeBgColor(display)) {
    ...
    for (token <- ScalaLexer.rawTokenise(text, forgiveErrors = true)) {
      ...
      val bgColor = Option(textAttribute.getBackground) getOrElse codeBgColor(display)
      appendText(token.text, textAttribute.getForeground, bgColor, textAttribute.getStyle, insertNewline = false)
    }
    appendText("\n", codeFgColor(display), codeBgColor(display), SWT.NORMAL, insertNewline = false)
  }
  ...
  protected def displayError(text: String) = displayPadded(null) {
    appendText(text, errorFgColor(display), null, SWT.NORMAL)
  }
