...
object ReplConsole {
  private val projectMap = new mutable.HashMap[IProject, ReplConsole]
  
  def getConsole(project: IProject): Option[ReplConsole] = projectMap.get(project)
  def addConsole(project: IProject, console: ReplConsole) = projectMap.put(project, console)
}
...
class ReplConsole(name: String, val process: IProcess, colorProvider: IConsoleColorProvider) 
  ...
  def insertText(text: String) {
	  getDocument.replace(getDocument.getLength, 0, text)
  }
}
