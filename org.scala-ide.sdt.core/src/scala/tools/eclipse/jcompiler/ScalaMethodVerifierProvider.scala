...
class ScalaMethodVerifierProvider extends IMethodVerifierProvider with Logger {
  ...
  private def findProjectOf(abstractMethod: MethodBinding): IProject = {
    ...
    debug("Found definition of `%s` in file `%s` of project `%s`".format(abstractMethod, qualifiedFileName, project.getName()))
      ...
      pc.askOption { () =>
        ...
        def findMethodOwnerSymbol(abstractMethod: MethodBinding) = {
          ...
          if (packageName.isEmpty())
            ...
            // COMPATIBILITY WITH 2.8: Replace below try/catch statement with 
            // `pc.definitions.getClassIfDefined((packageName + "." + typeName).toTypeName)`
            // when 2.8 support is dropped. 
            try { pc.definitions.getClass((packageName + "." + typeName).toTypeName) }
            catch { case _ => NoSymbol }
        }
