      ...
      t match {
        ...
        case ValDef(mods, name, tpt, rsh) => {
          val ch = if ((mods.flags & MUTABLE) == 0)
            ValNode(name.decodedName.toString(), parent, if(tpt.isEmpty) None else Some(tpt.toString()))
          else
            VarNode(name.decodedName.toString(), parent, if(tpt.isEmpty) None else Some(tpt.toString()))
          setPos(ch, t)
          ch.setFlags(mods.flags)
          parent.addChild(ch)
        }
        ...
        case DefDef(mods, name, tparamss, vparamss, tpt, rsh) =>
          vparamss.foreach(pl => pl.map(p => logger.info(p.name+"|"+p.tpt+"|"+p.mods)))
          logger.info("DefDef " + mods + "!" + name + "!" + tparamss + "!" + vparamss + "!" + tpt + "!" + name.decodedName)

          def typeList = {
            vparamss.map { x => x.map { v => v.toString()/*.replace(" = _","")*/ } }
            //vparamss.map { x => x.map { v => comp.declPrinter.showValType(v) } }
          }
