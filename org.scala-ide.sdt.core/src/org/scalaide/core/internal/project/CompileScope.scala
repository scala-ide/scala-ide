...
/**
 * Used to carry information about scope of sources compilation in given project.<br/>
 ...
 * There are defined (as a convension) 3 scopes:<ul>
 * <li>macros with default source path: <project root>/src/macros* and
 * especially for Play projects <project root>/conf* </li>
 * <li>tests with default source path: <project root>/src/test* and (because of
 * Play projects) <project root>/test* </li>
 * <li>main which gets all source paths which don't match macros and tests scopes
 * (* is a wildcard so for example to tests scope are assigned sources from
 * folders src/test, src/test-my-special)</li></ul>
 * <br/>
 ...
 * projects' scopes:<ul>
 * <li>macros of B depends on macros and main of A. So macros of B requires the
 * correct compilations of macros and main scopes in project A</li>
 * <li>main of B depends on macros and main of A</li>
 * <li>tests of B depends on macros, main and tests of A</li></ul>
 * @see SbtScopesBuildManager implementation for
 * implementation details.
 * <br/>
 * The feature is controlled by 'useScopesCompiler' exposed on 'build manager'tab in
 * Scala->Compiler preferencies menu. If this flag is not set then compilation process
 * falls to @see ProjectsDependentSbtBuildManager old approach.
 */
trait CompileScope {
  ...
  /** <code>true</code> if source belongs to this compilation scope. */
