...
abstract class ExtractProposal(displayString: String, hightlightFrom: Int, highlightTo: Int, relevance: Int = 0)
...
object ExtractProposal {
  def getQuickAssistProposals(cu: ScalaCompilationUnit, selectionStart: Int, selectionEnd: Int): Array[ExtractProposal] = {
    val proposals = new ArrayBuffer[ExtractProposal]
    ...
    cu.withSourceFile { (file, compiler) =>
      ...
      refactoring.extractions.foreach { extraction =>
        ...
        proposals += new ExtractProposal(extraction.displayName, pos.startOrPoint, pos.endOrPoint, relevance) {
          def apply(doc: IDocument) = {
